...
def process_order(organizer: str, event: str, pseudo_id: str) -> HttpResponse:
    api_client = PretixApiClient(settings.PRETIX_ROOT_URL, organizer, event, settings.PRETIX_API_KEYS[organizer])
    ...
    order = api_client.get_orderpositions({'pseudonymization_id': pseudo_id})
    ...
    if len(order) == 1 and order[0].get('item') in product_ids:
        ...
        try:
            ...
            Order.objects.create(order_id=order[0].get('order'), position_id=order[0].get('positionid'),
                                 donation=donation)
            return HttpResponseRedirect(reverse_lazy('donor_registration', kwargs={'pk': donation.pk}))
    else:
        if event == 'gpn23':
            return process_order(organizer, 'gpn23merch', pseudo_id)
        return HttpResponseNotAllowed(permitted_methods='GET')


class PretixImportView(RedirectView):
    def get(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        return process_order(kwargs['organizer'], kwargs['event'], kwargs['pseudo_id'])
